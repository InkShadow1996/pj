...
$sql="select count(*) from tb_send";
$result =mysql_query($sql);
$row=mysql_fetch_row($result);
$total = $row[0];//获取最大页码数
/*echo "<script type='text/javascript'>alert('$total');</script>";
$boy = "select * from tb_send where send_bankuai = '0'";
$ce = mysql_query($boy);
$num=mysql_num_rows($ce);
...
$sqla="select * from tb_send order by send_id desc limit $start ,$perpage ";
...
<table width="600px" border="1" cellpadding="8" cellspacing="0" align="center">
 <tr class="title">
 <td colspan="3">房屋租售帖子 <span class="list">[<a style="color: white" href="start.php">返回</a> ]</span></td>
 </tr>
 <tr>
 ...
 ?>
 <tr>
 <td width="280px"><div><a href="tiezi.php?send_id=<?php echo $row['send_id']?>&send_subject=<?php echo $row['send_subject'];?>"</a><?php echo $row['send_subject'];?></div> </td>
 <td width="160px"><?php echo $row['send_user'] ?></td>
 <td width="160px"><?php echo $row['send_date']?></td>
 </tr>
 <tr>
 <td colspan="3">
 <?php }
...
<input name="sendhtml" type="button" value="发帖" onclick="javascrtpt:window.location.href='sendht.php'" />
 </td>
